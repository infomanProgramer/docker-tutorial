...
Iniciemos un nuevo contenedor usando la imagen base del nodo recien creado
...
Dentro del contenedor, ejecutamos el siguiente comando para crear un programa Node
...
En otra terminal, ejecuta el siguiente comando para guardar los cambios del contenedor como una nueva imagen
...
1. **docker container commit:** Este comando toma un contenedor en ejecución y lo "commitea", es decir, genera una nueva imagen Docker basada en el estado actual del contenedor.
2. **-c "CMD node app.js":** Esta opción permite modificar la instrucción CMD de la nueva imagen. El comando CMD especifica qué proceso se ejecutará por defecto cuando un contenedor basado en la imagen se inicie. En este caso, está configurado para que cuando inicies un contenedor a partir de la nueva imagen, se ejecute node app.js.
3. **-m "Add app":** Especifica un mensaje de commit, que describe los cambios que se están realizando en la imagen. Este es similar a un mensaje de commit en Git, y en este caso se usa para registrar que se ha "agregado la aplicación".
4. **app-container:** Este es el nombre o ID del contenedor desde el cual se está creando la nueva imagen.
5. **sample-app:** Este es el nombre que se le asignará a la nueva imagen generada.

Y la nueva imagen se ve cargarda tambien dentro de Docker Descktop:
...
Proporciona instrucciones al generador de imágenes sobre los comandos a ejecutar, los archivos a copiar, el comando de inicio y más.

### Instrucciones comunes

Algunas de las instrucciones mas comunes en un Dockerfile incluyen:

- FROM <image>: Expecifica la imagén base de la que la construcción extendera
- WORKDIR <path>: Esta instrucción especifica el directorio de trabajo o la ruta en la imagén donde los archivos son copiados y los comandos serán ejecutados
- COPY <host path> <image path>: Esta instrucción le dice al constructor que copie los archivos desde la anfitriona (host) y los ponga dentro de contenedor de imagen
- RUN: Esta instrucción le dice al constructor que ejecute el comando especifico
...
- USER <user o uid>: Esta instrucción establece el usuario por defecto para todas las instrucciones posteriores
- CMD ["<command>", "<arg1>"]: Esta instrucción establece el comando predeterminado que ejecutará un contenedor que utilice esta imagen.

### Generación de imagenes

A menudo, las imagenes son generadas usando un Dockerfile. El comando basico docker build puede verse de la siguiente forma:

```
docker build .
```

El punto final suministra la ruta o url del compilado en contexto. En esta ubicación el constructor encontrara el Dockerfile y otros archivos referenciados.

Cuando ejecutas una compilación, el constructor extrae una imagen base. Si es necesario y ejecuta las instrucciones del Dockerfile.

Con el comando anterior la imagen no tendra nombre, pero la salida mostrara el ID de la imagén
![alt text](image-9.png)

Con la anterior salida, puedes empezar un contenedor usando la imagen de referencia

```
docker run sha256:1634abd2d09e1bce8a1f0980f3beefccbdf977ae2724d99df2c5740b4db56f2c
```

### Etiquetado de imagenes

El etiquetado de imagenes es un método para dar a una imagen un nombre memorable. Sin embargo hay una estructura de nombre de una imagen. Todo elnombre de la imagen se veria de la siguiente forma:

```
[HOST[:PORT_NUMBER]/]PATH[:TAG]
```

- **HOST:** El nombre de host del registro opcional donde se encuentra la imagen. Si el host no es especificado, el registro público de Docker **docker.io** será usado por defecto
- **PORT_NUMBER:** El registro de número de puerto si el host lo provee
- **PATH:** La ruta de la imagen, que consiste de componentes separados por slash. Para Docker Hub, el formato sigue:
  - [NAMESPACE/]REPOSITORY: donde el NAMESPACE puede ser el nombre de usuario o el nombre de la organización. Si no se especifica esta el NAMESPACE, se usara **library**, el eucla es el NAMESPACE para imagenes oficiales de Docerk
- **TAG:** Es un idenfificador personalizado entendible para el humano, que es comunmente usado para identificar versiones o variantes de una imagén. Si tag no es especificado **latest** sera usado por defecto

Para etiqueta una imagen durante la generación, añade -t o --tag:

```
docker build -t my-username/my-image .
```

Si ya has construido tu imagen, puedes añadir otro tag a la imagen usando **docker image tag**
...
Una vez que ya tienes una imagen compilada y etiquetada, estas listo para publicarlo en un registro, usando el comando:
